...
class Role extends Base
{
    ...
    public function data()
    {
        ...
        return json_data('role',$where,$data);
    }
    ...
    public function form()
    {
        if(request()->isPost()){
            ...
            if(!empty($post['id'])){
                ...
                unset($post['id']);
                $post['show_order']=Db::name('role')->where('pid',session('admin_role_id'))->max("show_order")+1;
                $post['pid']=session('admin_role_id');
                $post['create_time']=date('Y-m-d H:i:s');
                $res = Db::name('role')->insertGetId($post);
                if (!empty($res)) {
            ...
            $info = Db::name('role')->where('id', request()->get('id'))->find();
        ...
        if(request()->isPost()){
            ...
            */
            foreach ($fun_id as $k_threeid => $v_threeid) {
                ...
                */
                $permiss_twoid = Db::name('permission')->where("id",'in',$v_threeid)->value('pid');
            ...
            */
            ...
            foreach ($two_id as $k_two => $v_two) {
                ...
                * 根据二级菜单id 来获取 其相应参数
                */
                $per_arr = Db::name('permission')->where("id",'in',$v_two)->field('id,pid,con_name')->select();
                $per_arr = $per_arr[0];
                /*
                * 获取二级菜单的上级id
                */
            ...
            */
            ...
            foreach ($one_id as $k_oneid => $v_oneid) {
                ...
                */
                $two_ids_arr = Db::name('permission')->where("pid",'in',$v_oneid)->order("show_order asc")->field('id')->select();
                ...
                foreach ($two_ids_arr as $k_tia => $v_tia) {
                    ...
                    */
                    if (in_array($v_tia['id'],$two_id)) {
                        ...
                        */
                        $fun_ids_arr = Db::name('permission')->where("pid",'in',$v_tia['id'])->order("show_order asc")->field('id')->select();
            ...
            */
            ...
            adminLog($action,Db::name('role')->getLastSql());
            ...
            if (session('admin_role_id') != session('admin_super_role_id')) {
                ...
                $user_role_ids = Db::name('role')->where(array('pid'=>$user_role_id))->column('id');
            ...
            */
            ...
            if (!empty($role_info['pid'])) {
                ...
                * 如果不是超级管理员时
                */
                $user_permis_ids = db('role')->where(array('id'=>$role_info['pid']))->value('permis_ids');
                /*
                * 以 "|" 符号分割为数组，得到的数组就是 每个一级权限下的数据
                */
                ...
                foreach ($user_one_arr as $k_uoa => $v_uoa) {
                    ...
                    // 分割数组后的第一个数组为 一级权限，将本数组 赋值到 $one_ids 数组中
                    $auth_infoA[] = Db::name('permission')->where(array('id'=>$user_two_arr[0]))->field('id,name')->select()[0];
                    //删除 一级权限
                    ...
                    foreach ($user_two_arr as $k_uta => $v_uta) {
                        ...
                        * 分割数组后的第一个数组为 一级权限，将本数组 赋值到 $one_ids 数组中
                        */
                        $auth_infoB[] = Db::name('permission')->where(array('id'=>$user_fun_arr[0]))->field('id,name,pid')->select()[0];;
                        /*
                        * 删除 二级权限
                        */
                            ...
                            foreach ($user_function_arr as $k_ufas => $v_ufas) {
                                ...
                                */
                                if (!empty($v_ufas) && isset($v_ufas)) {
                                    $auth_infoC[] = Db::name('permission')->where(array('id'=>$v_ufas))->field('id,name,pid')->select()[0];;
                                }
                            }
                        }
                    }
                ...
                * 一级权限 数据
                */
                $auth_infoA = Db::name('permission')->where("pid='0'")->order("show_order asc")->field('id,name')->select();

                /*
                * 遍历一级权限，得到 所有一级权限的 ids  =》 二级权限
                */
                foreach ($auth_infoA as $k_aiA => &$v_aiA) {
                    $v_aiA['num']=Db::name('permission')->where('pid',$v_aiA['id'])->count();
                    $auth_infoA_ids[] = $v_aiA['id'];
                }
                $auth_infoB = Db::name('permission')->where('pid','in',$auth_infoA_ids)->order("show_order asc")->field('id,name,pid')->select();
                /*
                ...
                */
                ...
                $auth_infoC = Db::name('permission')->where('pid','in',$auth_infoB_ids)->order("show_order asc")->field('id,name,pid')->select();
            }
    ...
    {
        $post = request()->post();
        $res = Db::name('role')->delete($post['ids']);
        if(!empty($res)){
